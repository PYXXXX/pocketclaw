...
void main() {
  ...
  test(
    'AgentSessionCardViewData falls back to main agent when list is empty',
    () {
  ...
  test(
    'AgentSessionCardViewData sorts current session first, then selected agent matches',
    () {
      final viewData = AgentSessionCardViewData.from(
        strings: AppStrings.fromLocale(en),
        agents: const <AgentSummary>[
  ...
  test(
    'AgentSessionCardViewData reports hidden gateway session count after truncation',
    () {
      final gatewaySessions = List<SessionInfo>.generate(
        10,
        (index) =>
            SessionInfo(key: 'agent:main:pc-$index', label: 'Session $index'),
      );
