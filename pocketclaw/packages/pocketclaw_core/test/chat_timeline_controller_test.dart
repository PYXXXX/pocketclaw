...
void main() {
  group('ChatTimelineController', () {
    test('merges assistant deltas into a single streaming item', () {
      ...
      controller.applyChatStreamEvent(
        ChatStreamEvent(
          ...
          runId: 'run-1',
          message: ChatMessage(role: ChatMessageRole.assistant, text: 'Hel'),
        ),
      );
      controller.applyChatStreamEvent(
        ChatStreamEvent(
          ...
          state: ChatStreamState.delta,
          runId: 'run-1',
          message: ChatMessage(role: ChatMessageRole.assistant, text: 'lo'),
        ),
      );
      controller.applyChatStreamEvent(
        ChatStreamEvent(
          ...
          runId: 'run-1',
          message: ChatMessage(role: ChatMessageRole.assistant, text: 'Hello'),
        ),
      );
    ...
    test(
      'marks streaming assistant item as aborted and keeps partial text',
      () {
        final controller = ChatTimelineController();
    ...
    test(
      'marks streaming assistant item as error and appends system error text',
      () {
        final controller = ChatTimelineController();
