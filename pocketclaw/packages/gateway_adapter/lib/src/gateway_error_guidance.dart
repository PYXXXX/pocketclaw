...
final class GatewayErrorGuidance {
  const GatewayErrorGuidance({required this.summary, this.action, this.code});
...
GatewayErrorGuidance gatewayErrorGuidanceFor(
    ...
    switch (code) {
      case GatewayErrorCodes.pairingRequired:
        return const GatewayErrorGuidance(
          ...
          summary:
              'This device must be approved by the Gateway before it can connect.',
          action:
              'Approve the pending device pairing request from an existing trusted OpenClaw operator session or on the Gateway host.',
        );
      ...
      case GatewayErrorCodes.authPasswordNotConfigured:
        return const GatewayErrorGuidance(
          ...
          action:
              'Verify the password or switch to another supported auth path such as token or approved device token.',
        );
      case GatewayErrorCodes.authRateLimited:
        return const GatewayErrorGuidance(
          ...
          action:
              'Wait briefly before retrying, then verify the configured auth method.',
        );
      case GatewayErrorCodes.controlUiOriginNotAllowed:
        return const GatewayErrorGuidance(
          ...
          action:
              'Use the app from an allowed origin/network path or adjust Gateway origin policy on the host.',
        );
      ...
      case GatewayErrorCodes.deviceIdentityRequired:
        return const GatewayErrorGuidance(
          ...
          action:
              'Keep device auth enabled and ensure the app can persist a stable device identity.',
        );
      ...
      case GatewayErrorCodes.authRequired:
        return const GatewayErrorGuidance(
          ...
          action:
              'Provide a valid token, password, or previously issued device token.',
        );
      case GatewayErrorCodes.authDeviceTokenMismatch:
        return const GatewayErrorGuidance(
          summary: 'The stored device token is no longer accepted.',
          action:
              'Reconnect with another supported auth method and re-approve this device if necessary.',
        );
    }
  ...
  if (lower.contains('connection refused')) {
    if (_usesLoopback(configuredUrl)) {
      return const GatewayErrorGuidance(
        summary:
            'The app tried to connect to localhost / loopback, which on a phone means the phone itself.',
        action:
            'Use your Gateway host IP, LAN hostname, Tailscale name, or public domain instead of 127.0.0.1 / localhost.',
      );
    }
    return const GatewayErrorGuidance(
      summary: 'The Gateway socket refused the connection.',
      action:
          'Check that the Gateway is running, the host and port are correct, and that this phone can reach that network path.',
    );
  }
  ...
  if (lower.contains('cloudflare access redirected') ||
    ...
    return const GatewayErrorGuidance(
      summary:
          'Cloudflare Access redirected the request into an interactive login flow before PocketClaw could upgrade to WebSocket.',
      action:
          'Make sure the Access application explicitly allows the configured service token for this host/path, or exempt the Gateway WebSocket path/domain from Access.',
    );
  }
  ...
  if (lower.contains('rejected the provided service token')) {
    return const GatewayErrorGuidance(
      ...
      action:
          'Verify the Client ID / Client Secret pair, the Access policy binding, and whether the protected application includes this exact host/path.',
    );
  }
  ...
  if (lower.contains('403') || lower.contains('forbidden')) {
    return const GatewayErrorGuidance(
      summary:
          'The WebSocket handshake was rejected before PocketClaw could reach the Gateway.',
      action:
          'If this host is protected by Cloudflare Access, configure a service token and provide its Client ID / Client Secret in the app, or exempt the Gateway WebSocket path/domain from Access.',
    );
  }
  ...
  if (lower.contains('timed out')) {
    return const GatewayErrorGuidance(
      ...
      action:
          'Verify the URL, network reachability, and any reverse proxy or TLS configuration in front of the Gateway.',
    );
  }
